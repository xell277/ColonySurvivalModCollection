    ...
    public bool TryDoCommand (Players.Player causedBy, string chattext, List<string> splits)
    {
      ...
      if (accesslevel.Equals ("area")) {
        ...
        if (rangex.Length > 0 && int.TryParse (rangex, out rx) && rangez.Length > 0 && int.TryParse (rangez, out rz)) {
          AntiGrief.AddCustomArea (new CustomProtectionArea (causedBy.PositionVoxelStanding, rx, rz));
          Chat.Send (causedBy, $"Added anti grief area at {causedBy.PositionVoxelStanding} with x-range {rx} and z-range {rz}");
          ...
          AntiGrief.AddCustomArea (new CustomProtectionArea (causedBy.PositionVoxelStanding, rxn, rxp, rzn, rzp));
          Chat.Send (causedBy, $"Added anti grief area at {causedBy.PositionVoxelStanding} from x- {rxn} to x+ {rxp} and from z- {rzn} to z+ {rzp}");
        } else {
        ...
        if (accesslevel.Equals ("spawn")) {
          ...
          PermissionsManager.AddPermissionToUser(causedBy, targetPlayer, AntiGrief.PERMISSION_BANNER_PREFIX + causedBy.ID.SteamID.m_SteamID);
          ...
          PermissionsManager.RemovePermissionOfUser(causedBy, targetPlayer, AntiGrief.PERMISSION_BANNER_PREFIX + causedBy.ID.SteamID.m_SteamID);
